...
class FMD:
    ...
    def get_Mc(self, mc_type, **kwargs):
...
#        return ( 2.3 * np.sqrt((sum((self.data['mag']-meanMag)**2) ) / ( N * (N-1))) ) * self.par['b']**2
    ...
    def plotDistr(self, ax = None):
        ...
        ax.semilogy( self.data['magBins'], self.data['magHist'], 's',color='brown' ,ms = 2,alpha = 0.5, mew = 0.8, label = 'histogram')
        ax.semilogy( self.data['mag'], self.data['cumul'], linestyle='-', lw = 0.2, marker='o',color='gray',  ms = 5, alpha = 0.3,label='cumulative' )
    ...
    def plotFit(self, ax, color):
        ...
        #___________________________ distribution___________________________________________
        ax.semilogy( self.data['magBins'], self.data['magHist'], 's',color='brown', ms = 4, mew = 1,alpha = 0.6, label = 'histogram')
        sel  = self.data['mag'] > self.par['Mc'] - 1 # just one below completeness for fast plotting
        ax.semilogy( self.data['mag'][sel], self.data['cumul'][sel], linestyle='-',lw = 0.2, color = color, 
                    marker='o', ms = 5, #markeredgecolor = 'grey', 
        ...
        ax.semilogy( mag_hat, N_hat, '-',lw=3,color = 'white',
                     label='$log(N) = -%.1f \cdot M + %.1f$' %( round(self.par['b'],1), round( self.par['a'],1)) ,
                     alpha=0.8)
        ax.semilogy( mag_hat, N_hat, '-',lw=2,color = 'black',
                     label='$log(N) = -%.1f \cdot M + %.1f$' %( round(self.par['b'],1), round( self.par['a'],1)) ,
                     alpha=1)
        ax.plot( [self.par['Mc']], [self.data['cumul'][sel][0]], 'v', ms = 10,color = color, 
        ...
        ax.legend( shadow = False, numpoints=1, loc = 'upper right', fontsize = 10)#, frameon = False)
        #ax.set_xlim( self.par['Mc']-2, self.data['mag'].max()+.5)#ax.get_xlim()[1])
        ax.set_ylim(1, len( self.data['mag'])*1.5)
        ax.grid('on')
        return ax
    ...
    def plotKS(self, ax = None):
        ...
        if 'a_KS' in self.data.keys():
            ...
            ax.set_ylabel( 'KS-D')
            #ax.grid(True)
            #plt.savefig(f'{plotdir}/KS_D_plot.png', dpi = 1000, transparent= True)
